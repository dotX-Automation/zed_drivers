/**
 * Utility routines for the ZED 2i Driver node.
 *
 ...
 */
bool ZED2iDriverNode::open_camera()
{
  // Get node parameters
  ...
  if (err != sl::ERROR_CODE::SUCCESS) {
    RCLCPP_FATAL(
      this->get_logger(),
      "ZED2iDriverNode::open_camera: Failed to open the camera (%d): %s",
      static_cast<int>(err),
  ...
  if (err != sl::ERROR_CODE::SUCCESS) {
    RCLCPP_FATAL(
      this->get_logger(),
      "ZED2iDriverNode::open_camera: Failed to enable positional tracking (%d): %s",
      static_cast<int>(err),
      sl::toString(err).c_str());
    zed_.close();
    return false;
  ...
  init_camera_info(
    left_info,
    left_info_,
    link_namespace_ + "zed2i_left_link");
  init_camera_info(
    right_info,
    right_info_,
    link_namespace_ + "zed2i_right_link",
 ...
 */
void ZED2iDriverNode::close_camera()
{
  if (zed_.isOpened()) {
    zed_.close();
  }
 ...
 */
Image::SharedPtr ZED2iDriverNode::frame_to_msg(cv::Mat & frame)
{
 ...
 */
cv::Mat ZED2iDriverNode::slMat2cvMat(sl::Mat & input)
{
 ...
 * @return OpenCV frame-holding data structure, pointing to the same data.
 */
cv::Mat ZED2iDriverNode::slMat2cvMatDepth(sl::Mat & input)
{
  // Since cv::Mat data requires a uchar* pointer, we get the uchar1 pointer
  // from sl::Mat::getPtr<T>()
  // cv::Mat and sl::Mat will share a single memory structure
 ...
 */
void ZED2iDriverNode::init_camera_info(
 ...
 */
bool ZED2iDriverNode::validate_depth_mode(const rclcpp::Parameter & p)
{
  ...
  if (depth_mode == "PERFORMANCE") {
    ...
    RCLCPP_ERROR(
      this->get_logger(),
      "ZED2iDriverNode::validate_depth_mode: Invalid depth_mode parameter: %s",
      depth_mode.c_str());
    return false;
  }
  return true;
 ...
 * @return True if the parameter is valid, false otherwise.
 */
bool ZED2iDriverNode::validate_fps(const rclcpp::Parameter & p)
{
  ...
  switch (fps) {
    ...
    default:
      RCLCPP_ERROR(
        this->get_logger(),
        "ZED2iDriverNode::validate_fps: Invalid fps parameter: %ld",
        fps);
      return false;
  }
}
 ...
 * @return True if the parameter is valid, false otherwise.
 */
bool ZED2iDriverNode::validate_resolution(const rclcpp::Parameter & p)
{
  ...
  if (resolution == "HD2K") {
    ...
    RCLCPP_ERROR(
      this->get_logger(),
      "ZED2iDriverNode::validate_resolution: Invalid resolution parameter: %s",
      resolution.c_str());
    return false;
  }
  return true;
